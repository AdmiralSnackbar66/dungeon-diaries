---
publish: true
published: 2025-05-31
tags:
  - club
  - dnd
skills:
  - Athletics
  - Performance
title: The Intramural Water-Dancing Club
---
**Skills:** [[3-Mechanics/CLI/skills#Athletics|Athletics]], [[3-Mechanics/CLI/skills#Performance|Performance]]

**Description:**
These swimming enthusiasts combine their athletic skills with the finesse of artists as they choreograph water dances to the latest popular songs.

#### Members
<!-- QueryToSerialize: LIST FROM "content" WHERE contains(extracurricular, "The Intramural Water-Dancing Club") -->
<!-- SerializedQuery: LIST FROM "content" WHERE contains(extracurricular, "The Intramural Water-Dancing Club") -->
- [[Larine Arneza]]
<!-- SerializedQuery END -->
